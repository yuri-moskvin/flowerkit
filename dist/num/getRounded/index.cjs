 ...
 */const getRounded=(num,places=2)=>{if(typeof num!=="number"||Number.isNaN(num))throw new TypeError("getRounded: num must be a valid number");if(typeof places!=="number"||!Number.isFinite(places)||places<0||places>100)throw new RangeError("getRounded: places must be a finite number between 0 and 100");const shift=`e+${places}`;const unshift=`e-${places}`;return Number(Math.round(Number(String(num)+shift))+unshift)};exports.getRounded=getRounded;
