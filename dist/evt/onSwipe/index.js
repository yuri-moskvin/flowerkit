...
/**
   ...
   */const listenerCtrl=(isBind=true)=>{const action=isBind?"addEventListener":"removeEventListener";el[action](events.start,listeners.start);el[action](events.move,listeners.move);el[action](events.end,listeners.end);if(isSupport.pointer&&isSupport.touch)el[action]("lostpointercapture",listeners.end)};listenerCtrl(true);el[settings.instanceName]={swipeHandlers:listeners,listenerCtrl:listenerCtrl,destroy:()=>{listenerCtrl(false);delete el[settings.instanceName]}}};export{onSwipe};
