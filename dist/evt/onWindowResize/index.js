 ...
 * */const onWindowResize=(cb,delay=300,isAutoInit=true)=>{ow(cb,ow.function);ow(delay,ow.optional.number);ow(isAutoInit,ow.optional.boolean);const handler=()=>{if(delay){const fn=getDebouncedFn(cb);fn()}else cb()};const addListener=()=>{window.addEventListener("resize",handler)};const removeListener=()=>{getWindow().removeEventListener("resize",handler)};if(isAutoInit)addListener();return{handler:handler,addListener:addListener,removeListener:removeListener}};export{onWindowResize};
