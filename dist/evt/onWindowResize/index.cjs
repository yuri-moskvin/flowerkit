 ...
 */const onWindowResize=(cb,delay=300,isAutoInit=true)=>{if(typeof cb!=="function")throw new TypeError("onWindowResize: cb must be a function");if(typeof delay!=="number"&&typeof delay!=="undefined")throw new TypeError("onWindowResize: delay must be a number if provided");if(typeof isAutoInit!=="boolean")throw new TypeError("onWindowResize: isAutoInit must be a boolean");const fn=delay?index.getDebouncedFn(cb,delay):cb;const handler=e=>{fn(e)};const addListener=()=>{ssrWindow.getWindow().addEventListener("resize",handler)};const removeListener=()=>{ssrWindow.getWindow().removeEventListener("resize",handler)};if(isAutoInit)addListener();return{handler:handler,addListener:addListener,removeListener:removeListener}};exports.onWindowResize=onWindowResize;
