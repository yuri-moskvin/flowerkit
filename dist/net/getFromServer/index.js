...
// Return race
return await Promise.race(queries).then((resp=>getResponse(resp).then((resp=>{if(isBubble)bubble(getDocument().documentElement,getFromServer.name,resp);return getSuccessResp(resp)}))),(reject=>getReject(reject)))};export{getFromServer};
