 ...
 *   });
 */const getExternalScript=(props={})=>{ow(props,ow.object.exactShape({isAsync:ow.optional.boolean,isDefer:ow.optional.boolean,src:ow.string.not.empty,type:ow.optional.string.not.empty,appendTo:ow.optional.object.validate((value=>({validator:isNode(value),message:()=>`The object must be node`}))),id:ow.optional.string.not.empty,crossorigin:ow.optional.string.not.empty}));const{isAsync:isAsync=true,isDefer:isDefer=false,src:src="",type:type,appendTo:appendTo=getDocument().body,id:id="",crossorigin:crossorigin}=props;return new Promise(((resolve,reject)=>{let isReady=false;const script=getDocument().createElement("script");script.src=src;script.async=isAsync;script.defer=isDefer;if(id)script.id=id;if(crossorigin)script.crossorigin=crossorigin;if(type)script.type=type;script.onerror=err=>{reject(err);script.onerror=null};script.onload=script.onreadystatechange=function(){if(!isReady&&(!this.readyState||this.readyState==="complete")){isReady=true;resolve(script);script.onload=null;script.onerror=null}};appendTo.appendChild(script)}))};export{getExternalScript};
