 ...
 */const getElWrapper=(el,str)=>{if(!el||typeof el.nodeType!=="number")throw new TypeError("getElWrapper: el must be an HTMLElement");if(typeof str!=="string"||str.length===0)throw new TypeError("getElWrapper: str must be a non-empty string");const temp=ssrWindow.getDocument().createElement("div");const parent=el.parentNode;const insertWhere=el.previousSibling;let target;temp.innerHTML=str;target=temp.firstChild;while(target.firstChild)target=target.firstChild;target.appendChild(el);if(parent)parent.insertBefore(temp.firstChild,insertWhere?insertWhere.nextSibling:parent.firstChild);return target};exports.getElWrapper=getElWrapper;
